...
# Dedicated speech-to-text models. These are NOT general-purpose: they take audio and
# return a transcript, and cannot follow instructions or write notes. They are therefore
# offered on the transcription selectboxes only, never for notes, summary or analysis.
TRANSCRIPTION_ONLY_MODELS = {
    "Gemini 3.5 Transcribe (dedicated ASR)": "gemini-3.5-transcribe",
}
TRANSCRIPTION_MODELS = dict(MODELS)
TRANSCRIPTION_MODELS.update(TRANSCRIPTION_ONLY_MODELS)

# Segmentation for the dedicated ASR path. It accepts 30 minutes per request with
# diarization enabled, so windows are far wider than the LLM path's 5 minutes - an hour
# of audio goes from 11 seams to 2.
ASR_SEGMENT_SECONDS = 1500          # 25 min, inside the 30-min diarization limit
ASR_OVERLAP_SECONDS = 20


def is_transcription_only_model(model_id: str) -> bool:
    return model_id in TRANSCRIPTION_ONLY_MODELS.values()
...
def transcription_model_index(display_name: str) -> int:
    """Selectbox index within the transcription list (general models + dedicated ASR)."""
    keys = list(TRANSCRIPTION_MODELS.keys())
    return keys.index(display_name) if display_name in keys else 0
...
MODEL_PRICING = {
    ...
    # Dedicated ASR, billed per minute of audio in Google's own table
    # ($0.003/min in + $0.002/min out); the per-1M equivalents are used here so the
    # existing token-based cost panel keeps working.
...
-   **PRIORITY #1: CAPTURE ALL HARD DATA.** This includes all names, monetary values (`$`, `₹`, including crore and lakh figures), percentages (`%`), revenue/EBITDA/margin figures, growth rates, capex, capacity, store counts, market shares, customer counts, named segments, brand names, product names, customer names, geographies, and time periods. Reproduce figures in the unit the speaker used - never convert, never round.
...
def get_model(display_name: str) -> "_ModelHandle":
    ...
    model_id = TRANSCRIPTION_MODELS.get(display_name, "gemini-3.7-flash")
...
def _segment_audio(input_path: str, duration: float,
                   segment_seconds: int = None, overlap_seconds: int = None) -> List[str]:
    ...
    segment_seconds = segment_seconds or AUDIO_SEGMENT_SECONDS
    overlap_seconds = overlap_seconds if overlap_seconds is not None else AUDIO_OVERLAP_SECONDS
...
def _custom_vocabulary_from_context(context: str, limit: int = 100):
    """Turn the free-text context box into custom-vocabulary terms for the ASR model.

    The box is meant for domain terms - company names, people, products - so entries are
    read off commas and newlines. Anything longer than five words is prose rather than a
    term and is skipped. Returns [] when nothing usable is found, which is a valid config."""
    terms, seen = [], set()
    for raw in re.split(r"[,\n;]+", context or ""):
        term = raw.strip(" \t-•*").strip()
        if not term or len(term.split()) > 5 or len(term) < 2:
            continue
        key = term.lower()
        if key in seen:
            continue
...
def _normalise_for_overlap(words):
    """Normalised tokens for seam matching, plus their index in the original list.

    Speaker labels are dropped: diarization numbers each request independently, so the
    same person can be "Speaker 1" in one segment and "Speaker 2" in the next. Matching
    on labels would fail on every seam."""
    out = []
    for i, word in enumerate(words):
        if word.endswith(":") or word.lower().startswith("speaker"):
            continue
...
def _join_on_overlap(previous: str, nxt: str, max_overlap_words: int = 80,
                     min_match_words: int = 5) -> str:
    """Trim from `nxt` any opening words already present at the end of `previous`.

    The dedicated ASR model cannot be told to resume where the last segment stopped, so
    the deliberate audio overlap produces duplicated words at the seam. This finds the
    longest run of words ending `previous` that also opens `nxt` and cuts it. If no
    confident match is found the text is returned untouched: a repeated sentence is a
    blemish, a dropped one is a defect, and only one of those is acceptable."""
    prev_words, next_words = previous.split(), nxt.split()
    if not prev_words or not next_words:
        return nxt
    prev_norm = _normalise_for_overlap(prev_words)
    next_norm = _normalise_for_overlap(next_words)
    if not prev_norm or not next_norm:
        return nxt
    window = min(max_overlap_words, len(prev_norm), len(next_norm))
    for size in range(window, min_match_words - 1, -1):
        if [t for t, _ in prev_norm[-size:]] == [t for t, _ in next_norm[:size]]:
            cut_at = next_norm[size - 1][1] + 1     # index just past the matched run
            return " ".join(next_words[cut_at:]).lstrip()
    return nxt


def _transcribe_with_asr(model_id: str, cloud_file, custom_vocabulary) -> str:
    """One transcription call against a dedicated speech-to-text model.

    Uses the Interactions API rather than generate_content, and verbatim mode with
    speaker diarization - verbatim because the refinement stage already handles
    disfluencies and translation, so a raw transcript keeps the two paths comparable."""
    ...
    interaction = _client.interactions.create(
        model=model_id,
        input=[{"type": "audio", "uri": cloud_file.uri, "mime_type": cloud_file.mime_type}],
        generation_config={"transcription_config": transcription_config},
    )
    _record_usage(model_id, interaction, "Transcription")
...
def transcribe_audio(audio_bytes: bytes, model, status_write, context: str = "", file_ext: str = ".audio") -> str:
    ...
    model_id = getattr(model, "model_name", "") or ""
    asr_mode = is_transcription_only_model(model_id)
    custom_vocabulary = _custom_vocabulary_from_context(context) if asr_mode else []
    ...
    def _upload_and_transcribe(path: str, instruction: str) -> str:
        cloud = _upload(path)
        if asr_mode:
            return _transcribe_with_asr(model_id, cloud, custom_vocabulary)
    ...
    try:
        ...
        window = ASR_SEGMENT_SECONDS if asr_mode else AUDIO_SEGMENT_SECONDS
        overlap = ASR_OVERLAP_SECONDS if asr_mode else AUDIO_OVERLAP_SECONDS
        segments = _segment_audio(input_path, duration, window, overlap) if duration > window else []
        ...
        for i, seg_path in enumerate(segments):
            ...
            if asr_mode:
                # A dedicated ASR model takes no instructions, so the overlap is removed
                # afterwards by matching text rather than by asking the model to resume.
                text = _upload_and_transcribe(seg_path, "")
                transcripts.append(_join_on_overlap(transcripts[-1], text) if transcripts else text)
                continue
            if i == 0:
...
def page_process():
    ...
    with st.sidebar:
        ...
        transcription_model_name = st.selectbox(
            "Transcription model", list(TRANSCRIPTION_MODELS.keys()),
            index=transcription_model_index(DEFAULT_TRANSCRIPTION_MODEL),
...
def main():
    st.set_page_config(page_title="SynthNotes Pro · ASR test", layout="wide", page_icon="🎙")
